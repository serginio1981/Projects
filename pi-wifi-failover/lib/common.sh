#!/usr/bin/env bash
# lib/common.sh — utilidades propias de pi-wifi-failover: logging, dry-run y
# guards. Librería autocontenida: se carga con `source`, no se ejecuta, y no
...
# run CMD ARGS... — ejecuta, o solo muestra con DRY_RUN=1. Solo para comandos
# que modifican el sistema. OJO: no usar con argumentos secretos (contraseñas);
# para eso cada script maneja el dry-run a mano con el secreto enmascarado.
run() {
  if [[ "${DRY_RUN}" == "1" ]]; then
    log_dry "$(printf '%q ' "$@")"
    return 0
